...
class AtlasMerge:
    ...
    def merge_masks_without_overlap(self) -> npt.NDArray[np.uint16]:
        ...
        if np.any(ties):
            ...
            centers_of_mass = np.vstack(
                [*map(scipy.ndimage.center_of_mass, self.masks)]
            )
            ...
            for coordinate in zip(*np.nonzero(ties)):
                ...
                winner_index = indices[np.argmin(distance)]  # type: ignore[call-overload]
                warn(
    ...
    def from_templateflow(self, in_prefix: str, **kwargs: str):
        ...
        in_atlas_img = nib.nifti1.load(in_atlas_path)
    ...
    def from_file(
        ...
        if space is None:
            in_atlas_file = File(
                datatype="anat", path=str(in_atlas_path), metadata=dict()
            )
        ...
        in_atlas_img = nib.nifti1.load(result.outputs.output_image)
        in_atlas = np.asanyarray(in_atlas_img.dataobj, dtype=np.uint16)
